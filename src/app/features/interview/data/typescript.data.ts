...
function render(state: State) {
...

  {
    ...
    answer: `Mapped types transform properties of existing types. They iterate over keys and create new properties.

**Syntax:** { [K in Keys]: NewType }

**Modifiers:**
- readonly / -readonly — add/remove readonly
- ? / -? — add/remove optional

**Key remapping (as):** Transform key names.

**Use cases:** Making all props optional, readonly, nullable, filtering properties.`,
    codeSnippets: [
      {
        language: 'typescript',
        code: `// Basic mapped type — how Partial works
type MyPartial<T> = {
  [K in keyof T]?: T[K];
};
...
// Nullable — make all props nullable
type Nullable<T> = {
...
// { id: number | null; name: string | null }`,
      ...
      {
        language: 'typescript',
        code: `// Modifiers — add or remove
type Mutable<T> = {
  -readonly [K in keyof T]: T[K]; // Remove readonly
};

type Concrete<T> = {
  [K in keyof T]-?: T[K]; // Remove optional (how Required works)
};
...
// { getName: () => string; getAge: () => number }`,
      ...
      {
        language: 'typescript',
        code: `// Filter properties by type
type OnlyStrings<T> = {
...
type Strings = OnlyStrings<Mixed>; // 'a' | 'b'`,
...

  {
    ...
    answer: `Template literal types build string types from other types using template syntax.

**Syntax:** \`prefix\${Type}suffix\`

**Intrinsic string types:**
- Uppercase<S>, Lowercase<S>
- Capitalize<S>, Uncapitalize<S>

**Use cases:** Event names, CSS properties, route patterns, API endpoints.`,
    codeSnippets: [
      {
        language: 'typescript',
...
// Union expansion — all combinations
...
setWidth('100px');   // OK
setWidth('1.5rem');  // OK
setWidth('100');     // Error — no unit
setWidth('100vw');   // Error — vw not in union
...
user.on('nameChanged', (v) => {}); // v is string
user.on('ageChanged', (v) => {});  // v is number`,
      },
    ],
  },
...
// toggle: () => void`,
  ...
  {
    id: 'ts-10',
    question: 'Explain Function Overloads',
    answer: `Overloads define multiple signatures for one function. TypeScript picks the right one based on arguments.

**Structure:**
1. Overload signatures (declarations) — what callers see
2. Implementation signature — must handle all cases

**Rules:**
- Implementation signature must be compatible with all overloads
- Overloads checked top to bottom — put specific ones first
- Implementation signature is NOT visible to callers

**Use cases:** Different return types based on input, optional params with different behaviors.`,
    codeSnippets: [
      {
        language: 'typescript',
        code: `// Problem: return type depends on input
...
const str = process('hello'); // string (not string | number)
const num = process(42);      // number (not string | number)

// Without overloads, both would be string | number`,
      },
      {
        language: 'typescript',
        code: `// Practical: createElement-style API
...
const input = createElement('input'); // InputElement`,
      ...
      {
        language: 'typescript',
        code: `// Overload with different param counts
